...
class AblationGPT2Attention(GPT2Attention):
...
class AblationGPT2Block(GPT2Block):
    def __init__(self,config, layer_idx = None,
                 custom_attention_class = AblationGPT2Attention,
                 ablation_head_idx: dict = None):
        super().__init__(config, layer_idx)
        self.attn = custom_attention_class(config=config, layer_idx=layer_idx,
                                           ablation_head_idx=ablation_head_idx)
        self.layer_idx = layer_idx
        self.ablation_head_idx = ablation_head_idx
        if config.add_cross_attention:
            self.crossattention = custom_attention_class(config=config, is_cross_attention=True, layer_idx=layer_idx)
...
class AblationGPT2Model(GPT2Model):
    def __init__(self, config,
                 ablation_head_idx: dict = None):
        super().__init__(config)
        self._attn_implementation = None  # we are using custom attention
        self.h = nn.ModuleList([AblationGPT2Block(config,
                                                  layer_idx=i,
                                                  ablation_head_idx=ablation_head_idx
...
class AblationGPT2LMHeadModel(GPT2LMHeadModel):
    _tied_weights_keys = ["lm_head.weight"]
    def __init__(self, config,
                 ablation_head_idx: dict = None):
        super().__init__(config)
        self.transformer = AblationGPT2Model(config, ablation_head_idx=ablation_head_idx)
...
#     seq_len = 50
#     rep = 2
#     model = AblationGPT2LMHeadModel.from_pretrained('gpt2', ablation_head_idx={5:[3,4,1], 8:[1,2,3,4]})
...
#         outputs = gpt2(repeated_tokens)
#         # outputs = model.forward_plus(repeated_tokens, ablation_mode='full')
#         # outputs = model.forward_plus(repeated_tokens, ablation_mode='pp')
