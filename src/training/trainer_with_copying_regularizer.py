...
class TrainerWithCopyingRegularizer(Trainer):
    def __init__(
            self,
            model = None,
            args = None,
        ...
        self.reg_every_n_step = int(1/reg_lambda)
        self.device = device
    ...
    def _get_reg_val(self, attentions, span_lengths):
        ...
        seq = attentions.amax(dim=(1, 2))  # (batch_size, num_tok, num_tok)
        batch_size, ctx_size, _ = seq.shape

        # Step 3: Create the indices tensor for loss calculation
        # bases = [([1]+[0]*(span_length-1))*(ctx_size//span_length+1) for span_length in span_lengths]
        # indices = torch.tensor(
        #     [[([0]*((src+1)%span_lengths[i])+base)[:ctx_size]
        ...
        # Step 3: Create the indices tensor for loss calculation
        indices = torch.zeros(len(span_lengths), ctx_size, ctx_size, device=attentions.device, dtype=torch.int)

        for i, span_length in enumerate(span_lengths):
            base = torch.zeros(ctx_size, device=attentions.device, dtype=torch.int)
            base[::span_length] = 1  # Set every `span_length`th position to 1

            for src in range(ctx_size):
                shift = (src + 1) % span_length
                shifted = torch.zeros_like(base)
                shifted[shift:] = base[:ctx_size-shift]
                indices[i ,src] = shifted
                # indices[i, src] = torch.roll(base, shift)
    ...
    def compute_loss(self, model, inputs, reg_inputs, return_outputs=False, regularize=True):
        """
        ...
        if regularize:
            ...
            # Now add copying regularization - suppresses attention weights corresponding to copying behavior
            reg_outputs = model(
                input_ids=reg_inputs["input_ids"],
                attention_mask=reg_inputs["attention_mask"],
                output_attentions=True,
                return_dict=True,
            )
            reg = self._get_reg_val(reg_outputs.attentions, reg_inputs['span_length'])
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                regularize = True if step % self.reg_every_n_step == 0 else False
                ...
                if (
                    ...
                    with model.no_sync():
                        tr_loss_step, tr_reg_step = self.training_step(
                            model, inputs, reg_inputs, regularize=regularize
                        )
                else:
                    tr_loss_step, tr_reg_step = self.training_step(
                        model, inputs, reg_inputs, regularize=regularize
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    self._maybe_log_save_evaluate(
                        tr_loss, tr_reg, model, trial, epoch, ignore_keys_for_eval, log_reg=regularize
                    )
                else:
    ...
    def training_step(self,
        ...
        with self.compute_loss_context_manager():
            loss, reg = self.compute_loss(model, inputs, reg_inputs, regularize=regularize)
                ...
                if has_labels or loss_without_labels:
                    with self.compute_loss_context_manager():
                        loss, outputs, _ = self.compute_loss(
                            model, inputs, None, return_outputs=True, regularize=False
                        )
    ...
    def _maybe_log_save_evaluate(self, tr_loss, tr_reg, model, trial, epoch, ignore_keys_for_eval, log_reg):
